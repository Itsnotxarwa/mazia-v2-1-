import {useLocation} from "react-router-dom";
import { Mail, Phone, Handshake, MapPin } from "lucide-react";
import mazLogo from "./assets/image.png";
import LogoWhite from "./startup-components/assets/logo-white.png";

export default function Footer() {
        const location = useLocation();
    
        const isHomePage = location.pathname === "/";
        const isStartupPage = location.pathname === "/startup";
    return(
        <footer className="pt-24 pb-12 sm:pt-32 sm:pb-16">
            <div className="mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex flex-col md:flex-row space-y-6 gap-12 md:gap-24">
                    <div className='flex flex-col justify-between space-y-6'>
                        <div className='space-y-6'>
                            {isHomePage ? (
                                <img src={mazLogo} alt="Logo" className="w-44" />
                            ) : (
                                <img src={LogoWhite} alt="Logo" className="w-44" />
                            )}
                        
                        <p className='text-lg leading-relaxed'>
                            Votre assistant IA pour les appels téléphoniques.
                        </p>
                        </div>

                        <div className="card">
                            <a 
                            href="https://www.instagram.com/mazia_ai_callbot?igsh=ZWw4Mm9seGd4NjQz&utm_source=qr" 
                            target="_blank"
                            rel="noopener noreferrer"
                            className="socialContainer containerOne">
                                <svg className="socialSvg instagramSvg" viewBox="0 0 16 16"> <path d="M8 0C5.829 0 5.556.01 4.703.048 3.85.088 3.269.222 2.76.42a3.917 3.917 0 0 0-1.417.923A3.927 3.927 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297.04.852.174 1.433.372 1.942.205.526.478.972.923 1.417.444.445.89.719 1.416.923.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.916 3.916 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.926 3.926 0 0 0-.923-1.417A3.911 3.911 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0h.003zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046.78.035 1.204.166 1.486.275.373.145.64.319.92.599.28.28.453.546.598.92.11.281.24.705.275 1.485.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.47 2.47 0 0 1-.599.919c-.28.28-.546.453-.92.598-.28.11-.704.24-1.485.276-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.478 2.478 0 0 1-.92-.598 2.48 2.48 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485-.038-.843-.046-1.096-.046-3.233 0-2.136.008-2.388.046-3.231.036-.78.166-1.204.276-1.486.145-.373.319-.64.599-.92.28-.28.546-.453.92-.598.282-.11.705-.24 1.485-.276.738-.034 1.024-.044 2.515-.045v.002zm4.988 1.328a.96.96 0 1 0 0 1.92.96.96 0 0 0 0-1.92zm-4.27 1.122a4.109 4.109 0 1 0 0 8.217 4.109 4.109 0 0 0 0-8.217zm0 1.441a2.667 2.667 0 1 1 0 5.334 2.667 2.667 0 0 1 0-5.334z"></path> </svg>
                            </a>

                            <a 
                            href="https://x.com/maziaaicallbot?s=21" 
                            target="_blank"
                            rel="noopener noreferrer"
                            className="socialContainer containerTwo">
                                <svg className="socialSvg twitterSvg" viewBox="0 0 16 16"> <path d="M5.026 15c6.038 0 9.341-5.003 9.341-9.334 0-.14 0-.282-.006-.422A6.685 6.685 0 0 0 16 3.542a6.658 6.658 0 0 1-1.889.518 3.301 3.301 0 0 0 1.447-1.817 6.533 6.533 0 0 1-2.087.793A3.286 3.286 0 0 0 7.875 6.03a9.325 9.325 0 0 1-6.767-3.429 3.289 3.289 0 0 0 1.018 4.382A3.323 3.323 0 0 1 .64 6.575v.045a3.288 3.288 0 0 0 2.632 3.218 3.203 3.203 0 0 1-.865.115 3.23 3.23 0 0 1-.614-.057 3.283 3.283 0 0 0 3.067 2.277A6.588 6.588 0 0 1 .78 13.58a6.32 6.32 0 0 1-.78-.045A9.344 9.344 0 0 0 5.026 15z"></path> </svg>              
                            </a>
    
                            <a 
                            href="https://l.instagram.com/?u=https%3A%2F%2Fwww.linkedin.com%2Fcompany%2Fmaximize-infogerance%2Fposts%2F%3FfeedView%3Dall&e=AT0pH19kMekaMyfVuyVSkXz5xz8aNPA9ylMjH7a6FvEtHZMBfDTAJAZVIpKd1glIxryXb98KyEESVZ-NNKG-mWmShLGAST3I_rI2VRFqZ57OVip7juVmMzUwrOvO" 
                            target="_blank"
                            rel="noopener noreferrer"
                            className="socialContainer containerThree">
                                <svg className="socialSvg linkdinSvg" viewBox="0 0 448 512"><path d="M100.28 448H7.4V148.9h92.88zM53.79 108.1C24.09 108.1 0 83.5 0 53.8a53.79 53.79 0 0 1 107.58 0c0 29.7-24.1 54.3-53.79 54.3zM447.9 448h-92.68V302.4c0-34.7-.7-79.2-48.29-79.2-48.29 0-55.69 37.7-55.69 76.7V448h-92.78V148.9h89.08v40.8h1.3c12.4-23.5 42.69-48.3 87.88-48.3 94 0 111.28 61.9 111.28 142.3V448z"></path></svg>
                            </a>

                            <a 
                            href="https://web.facebook.com/MaximizeInfogerance" 
                            target="_blank"
                            rel="noopener noreferrer"
                            className="socialContainer containerFour">
                                <svg className="socialSvg FacebookSvg" viewBox="0 0 320 512" height="1.2em" xmlns="http://www.w3.org/2000/svg"> <path d="M279.14 288l14.22-92.66h-88.91v-60.13c0-25.35 12.42-50.06 52.24-50.06h40.42V6.26S260.43 0 225.36 0c-73.22 0-121.08 44.38-121.08 124.72v70.62H22.89V288h81.39v224h100.17V288z"></path></svg>
                            </a>
                        </div>             
                    </div>

                    <div className="grid grid-cols-2 md:grid-cols-3 space-y-4 md:justify-items-center w-full">

                    <div className="space-y-4">
                        <h3 className={`font-semibold text-lg ${isStartupPage ? "text-white" : "text-black/70"} tracking-tight underline`}>Menu</h3>
                        <ul className="space-y-3">
                            <li>
                                <a 
                                href="/"
                                className={`${isStartupPage ? "text-white" : "text-black/60"} text-lg leading-relaxed tracking-tight transition-colors hover:text-black`}>
                                    Accueil
                                </a>
                            </li>
                            <li>
                                <a 
                                href="/#fonctionnalities"
                                className={`${isStartupPage ? "text-white" : "text-black/60"} text-lg leading-relaxed tracking-tight transition-colors hover:text-black`}>
                                    Fonctionnalitiés
                                </a>
                            </li>
                            <li>
                                <a 
                                href="/#cas-usage"
                                className={`${isStartupPage ? "text-white" : "text-black/60"} text-lg leading-relaxed tracking-tight transition-colors hover:text-black`}>
                                    Cas d'usage
                                </a>
                            </li>
                            
                            <li>
                                <a 
                                href="/#tarifs"
                                className={`${isStartupPage ? "text-white" : "text-black/60"} text-lg leading-relaxed tracking-tight transition-colors hover:text-black`}>
                                    Tarifs
                                </a>
                            </li>
                            <li>
                                <a 
                                href="/#faq"
                                className={`${isStartupPage ? "text-white" : "text-black/60"} text-lg leading-relaxed tracking-tight transition-colors hover:text-black`}>
                                    FAQ
                                </a>
                            </li>
                            <li>
                                <a 
                                href="/#testimonials"
                                className={`${isStartupPage ? "text-white" : "text-black/60"} text-lg leading-relaxed tracking-tight transition-colors hover:text-black`}>
                                    Ils nous ont fait confiance
                                </a>
                            </li>
                            <li>
                                <a 
                                href="/contact"
                                target="_blank"
                                rel="noopener noreferrer"
                                className={`${isStartupPage ? "text-white" : "text-black/60"} text-lg leading-relaxed tracking-tight transition-colors hover:text-black`}>
                                    Contact
                                </a>
                            </li>
                            <li>
                                <a 
                                href="/startup"
                                target="_blank"
                                rel="noopener noreferrer"
                                className={`${isStartupPage ? "text-white" : "text-black/60"} text-lg leading-relaxed tracking-tight transition-colors hover:text-black`}>
                                    Start-up
                                </a>
                            </li>
                        </ul>
                    </div>

                    <div className="space-y-4">
                        <h3 className={`font-semibold text-lg ${isStartupPage ? "text-white" : "text-black/70"} tracking-tight underline`}>Legal</h3>
                        <ul className="space-y-3">
                            <li>
                                <a 
                                href="/mentions-légales"
                                className={`${isStartupPage ? "text-white" : "text-black/60"} text-lg leading-relaxed tracking-tight transition-colors hover:text-black`}>
                                    Mentions légales
                                </a>
                            </li>
                            <li>
                                <a 
                                href="/Politiques-de-confidentialité"
                                className={`${isStartupPage ? "text-white" : "text-black/60"} text-lg leading-relaxed tracking-tight transition-colors hover:text-black`}>
                                    Politiques de confidentialité
                                </a>
                            </li>
                            <li>
                                <a 
                                href="/Conditions-d-utilisation"
                                className={`${isStartupPage ? "text-white" : "text-black/60"} text-lg leading-relaxed tracking-tight transition-colors hover:text-black`}>
                                    Conditions d'utilisation
                                </a>
                            </li>
                        </ul>
                    </div>

                    <div className="space-y-4">
                        <h3 className={`font-semibold text-lg ${isStartupPage ? "text-white" : "text-black/70"} tracking-tight underline`}>Contact</h3>
                        <ul className="space-y-3">
                            <li className="flex gap-3 items-center">
                                <div className="flex justify-center p-2 border border-gray-500 bg-gray-50 rounded-lg text-black">
                                <Mail size={18} />
                                </div>
                                <a 
                                href=""
                                className="text-blue-800 text-lg leading-relaxed tracking-tight transition-colors hover:underline">
                                    contact@mazia.ai
                                </a>
                            </li>
                            <li className="flex items-center gap-3">
                                <div className="flex justify-center p-2 border border-gray-500 bg-gray-50 rounded-lg text-black">
                                <Phone size={18} />
                                </div>
                                <a 
                                href=""
                                className={`${isStartupPage ? "text-white" : "text-black/60"} text-lg leading-relaxed tracking-tight transition-colors whitespace-nowrap hover:underline`}>
                                    +33 189317006
                                </a>
                            </li>
                            <li className="flex items-center gap-3">
                                <div className="flex justify-center p-2 border border-gray-500 bg-gray-50 rounded-lg text-black">
                                <MapPin size={18} />
                                </div>
                                <a 
                                href=""
                                className={`${isStartupPage ? "text-white" : "text-black/60"} text-lg leading-relaxed tracking-tight transition-colors whitespace-nowrap hover:underline`}>
                                    61 rue de Lyon 75012 Paris 12
                                </a>
                            </li>
                            <li className="flex items-center gap-3">
                                <div className="flex justify-center p-2 border border-gray-500 bg-gray-50 rounded-lg text-black">
                                <Handshake size={18} />
                                </div>
                                <a href="/devenir-partenaire"
                                className={`${isStartupPage ? "text-white" : "text-black/60"} text-lg leading-relaxed tracking-tight transition-colors hover:text-black`}>
                                    Devenir Partenaire
                                </a>
                            </li>
                        </ul>
                    </div>

                    </div>
                    
                </div>
            </div>
        </footer>
    )
}